...
# --- MODEL SETUP (Lightweight Scikit-Learn Pipeline) ---
print('Loading Scikit-Learn model and vectorizer...')
try:
...
explainer = LimeTextExplainer(class_names=['Safe', 'Spam'])


def lime_predictor(texts):
  """Wrapper function for LIME to send batches of text to Scikit-Learn model."""
...
def explain_prediction(text, prediction):
  ...
  exp = explainer.explain_instance(
      text, lime_predictor, num_features=6, num_samples=100
  )
...
def run_text_inference(text):
  ...
  prediction_val = model.predict(transformed)[0]
  probabilities = model.predict_proba(transformed)[0]
  confidence = float(np.max(probabilities)) * 100

  # Map prediction (assuming 1 or 'Spam' is spam, 0 or 'Ham' is safe)
  if str(prediction_val) in ['1', 'Spam', 'phishing']:
    pred_label = 'Spam'
  else:
    pred_label = 'Safe'
...
def predict():
  ...
  cursor = conn.cursor()
  cursor.execute(
      'INSERT INTO scans (user_id, scan_type, prediction, confidence) VALUES'
      ' (?, ?, ?, ?)',
...
def predict_qr():
  ...
  try:
    ...
    if (
        extracted_text.startswith('http://')
        or extracted_text.startswith('https://')
...
def security_agent():
  ...
  if 'link' in query or 'url' in query:
    response = (
        'Link Guard Analysis: Check for mismatched domains, IP-based URLs, or'
        ' URL shorteners. You can test any link using our Link & URL Checker tab'
        ' above.'
    )
  elif 'email' in query or 'phish' in query:
    response = (
        'Email Phishing Insight: Generative AI makes phishing emails look'
        ' professional. Watch out for urgent calls-to-action, financial'
        ' requests, and spoofed sender headers.'
    )
  elif 'sms' in query or 'text' in query:
    response = (
        'Smishing Alert: Smishing texts often pretend to be package delivery or'
        ' bank alerts. Legitimate entities rarely send login links via standard'
        ' SMS numbers.'
    )
  elif 'qr' in query or 'code' in query:
    response = (
        'Qrishing Warning: Attackers embed malicious URLs inside QR codes to'
        ' bypass text filters. Always upload and inspect QR codes using our new'
        ' QR Scanner tab!'
    )
  elif 'lime' in query or 'explain' in query:
    response = (
        'Explainable AI (LIME): When our model flags an email as Spam, LIME'
        ' breaks down the text and highlights the exact trigger words that'
        ' caused the alert.'
    )
  else:
    response = (
        "AI Security Agent: I've processed your query. Paste suspicious text"
        ' into our Email/SMS tabs, enter a URL, or upload a QR code for'
        ' evaluation!'
    )
...
def admin_dashboard():
  ...
  cursor.execute(
      "SELECT COUNT(*) FROM scans WHERE prediction IN ('Spam', 'Phishing')"
  )
